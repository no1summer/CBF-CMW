...
%it is not the drifting that matters 
    ...
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\pure_freq'));
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\hist'));
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\complete_freq'));
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\complete_phase'));
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\pure_phase_gray'));
 ...
 for bas_num=startnum:endnum
    ...
    %imwrite (mat2gray(filteredSpectralImage2D),strcat(fileDirectory,'result_1.5std_3to10\','Amp',num2str(bas_num),'.tif'));
    ...
    imwrite (outputimg1,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\pure_freq\',fileName1,'pure_freq',num2str(bas_num),'.tif'));
    ...
    imwrite(outputimg2,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\complete_freq\','complete',num2str(bas_num),'.tif'));
    close(hFig);
    ...
    for t=2:size(uniquenumber)
        ...
        replace(location)=0;
        mkdir(strcat(fileDirectory,'result_1.5std\spectralImage\freq',num2str(uniquenumber(t))));
        imwrite (mat2gray(replace),strcat(fileDirectory,'result_1.5std\spectralImage\freq',num2str(uniquenumber(t)),'\Amplitude',num2str(bas_num),'.tif'));
    ...
    saveas(f, strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\hist\',fileName1,'hist',num2str(bas_num),'.tif'));
    close(f);
    
    
    for freq=6
        mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15_',num2str(freq),'Hz\pure_phase'));
    %uniquenumber=unique(frequencyImage);
    %for t=2:size(uniquenumber)
        %Indice = find(frequencyImage ~= uniquenumber(t));
        
                temp = angle(phasevol(:,:,:));
                phaseImage2D(:,:)=(temp(:,:,round(freq*NFFT/Fs)+1)+3.14).*binaryImage(:,:);
                orgimg2=ind2rgb(im2uint8(mat2gray(phaseImage2D)),parula);
                redChannel2 = uint8(orgimg2(:, :, 1)*256);
                greenChannel2 = uint8(orgimg2(:, :, 2)*256);
                blueChannel2 = uint8(orgimg2(:, :, 3)*256);
                backgroundpixel = redChannel2 == 62 & greenChannel2  == 39 & blueChannel2  == 169;
                redChannel2(backgroundpixel) = 0;
                greenChannel2(backgroundpixel) = 0;
                blueChannel2(backgroundpixel) = 0;
                outputimg2 = cat(3, redChannel2, greenChannel2, blueChannel2);   
    
        %phaseImage2D(Indice)=0;
        imwrite (outputimg2,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15_',num2str(freq),'Hz\pure_phase\',fileName1,'pure_phase_at_freq',num2str(freq),'Hz',num2str(bas_num),'.tif'));
        ...
        colorbar;
        [cdata,colorMap]=getframe(hFig);
        imwrite(cdata,strcat(fileDirectory,'128f_result_1.5std\threshold40_2to15\complete_phase\',num2str(bas_num),'_',num2str(uniquenumber(t)),'.tif'));
        close(hFig);
        
        imwrite(mat2gray(phaseImage2D),strcat(fileDirectory,'128f_result_1.5std\threshold40_2to15\pure_phase_gray\',num2str(bas_num),'_',num2str(uniquenumber(t)),'.tif'));
        %}
    end
 end 
