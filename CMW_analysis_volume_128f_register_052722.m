    ...
    fileDirectory = 'G:\800nm OCT computer data\052722 validation\biopshy_100x30_0.1x0.05_4us_800us_1\images\';
    fileName1='image_';
    fileName2 = '.tiff';
    ...
    mkdir(strcat(fileDirectory,'128f\threshold50_freq3\pure_freq_color'));
    mkdir(strcat(fileDirectory,'128f\threshold50\pure_freq_color'));
    ...
    mkdir(strcat(fileDirectory,'128f\threshold50_freq3\pure_phase_color_parula'));
 ...
 for j=0:128
    ...
    for X = 1:100
        for D = 1:512
            ...
            freqImage3D(D,X,z) = (index(1,1)+lowersignal-1)*Fs/L*binaryImage(D,X,z);
            %intensity(D,X,z)=max(frequencyAmplitudeProfile)*binaryImage(D,X,z);
                ...
                if binaryImage(D,X,z)==1
                  ...
                  phasevol(D,X,z,:)=phaseProfile(round(freq*L/Fs)+1)+pi;
   ...
   %}
    ...
    saveas(f, strcat(fileDirectory,'128f\threshold50\pure_freq_color\','hist',num2str(j),'.tif'));
    close(f);
    
    
    for z=1:30
    imwrite (ind2rgb(im2uint8(mat2gray(freqImage3D(:,:,z))),parula),strcat(fileDirectory,'128f\threshold50_freq3\pure_freq_color\',fileName1,'pure_T',num2str(j),'_Z',num2str(z),'.tif'));
    ...
    %imwrite (mat2gray(intensity(:,:,z)),strcat(fileDirectory,'128f\threshold50_freqindex1to30\pure_freq_intensity\','pure_T',num2str(j),'_Z',num2str(z),'.tif'));    
     ...
     for freq=4
    %imwrite (mat2gray(phasevol(:,:,z,l)),strcat(fileDirectory,'128f\threshold50_freqindex1to30\pure_phase\','pure_T',num2str(j),'_C',num2str(l),'_Z',num2str(z),'.tif'));
     imwrite (ind2rgb(im2uint8(mat2gray(phasevol(:,:,z,round(freq*L/Fs)+1))),parula),strcat(fileDirectory,'128f\threshold50_freq3\pure_phase_color_parula\',fileName1,'parula_T',num2str(j),'_C',num2str(freq),'_Z',num2str(z),'.tif'));
