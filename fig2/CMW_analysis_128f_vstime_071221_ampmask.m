    ...
    %progressbar(j/100);
        for fileNumber = 1:128
            ...
            Vol(:,:,fileNumber)= imread(strcat(fileDirectory,fileName1, num2str(frameNumber,'%05g'),fileName2));
    ...
    imwrite (outputimg1,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\pure_freq\pure_bright_field_pure_T',num2str(bas_num),'.tif'));
    ...
    imwrite(outputimg2,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\complete_freq\bright_field_complete_T',num2str(bas_num),'.tif'));
    close(hFig);
    ...
    saveas(f, strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\hist\hist_T',num2str(bas_num),'.tif'));
    close(f);
      ...
      for thresholding=0.7
       ...
       mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\ampmask_phase_',num2str(thresholding),'std\'));
       temp = angle(phasevol(:,:,:));
       ...
       redChannel3 = uint8(orgimg3(:, :, 1)*256);
        ...
        imwrite (outputimg3,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\ampmask_phase_',num2str(thresholding),'std\Positive_T',num2str(bas_num),'_C',num2str(uniquenumber(t)),'Hz.tif'));
       ...
       redChannel4 = uint8(orgimg4(:, :, 1)*256);
        ...
        imwrite (outputimg4,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\ampmask_phase_',num2str(thresholding),'std\Negative_T',num2str(bas_num),'_C',num2str(uniquenumber(t)),'Hz.tif'));
       %}
       ...
       redChannel4 = uint8(orgimg4(:, :, 1)*256);
        ...
        blueChannel4(backgroundpixel) = 0;
        outputimg4 = cat(3, redChannel4, greenChannel4, blueChannel4);
        imwrite (outputimg4,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold40_2to15\ampmask_phase_',num2str(thresholding),'std\black_T',num2str(bas_num),'_C',num2str(uniquenumber(t)),'Hz.tif'));
        end 
       end
        %{
    hFig=figure;
    ...
    colorbar;
    [cdata,colorMap]=getframe(hFig);
    imwrite(cdata,strcat(fileDirectory,'128f_1.5std\threshold130_4Hz\complete_phase\',fileName1,'\bright_field_complete_T',num2str(bas_num),'.tif'));
    close(hFig); 

    imwrite(mat2gray(phaseImage2D),strcat(fileDirectory,'128f_1.5std\threshold130_4Hz\pure_phase_gray\',fileName1,'\bright_field_pure_T',num2str(bas_num),'.tif'));
    %}
