    ...
    fileDirectory = 'F:\090722 temp ex vivo\a\090722_ampulla_1000x10000_1x0.1_7us_10ms_1l\images\';
    fileName1='image_';
    fileName2 = '.tiff';
    ...
    saveas(f, strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold70_2to15\hist\hist_T',num2str(bas_num),'.tif'));
    close(f);
      ...
      for thresholding=0.7
       ...
       mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold70_2to15\ampmask_phase_',num2str(thresholding),'std\'));
       temp = angle(phasevol(:,:,:));
        phaseImage2D(:,:)=(temp(:,:,t+1)+3.14).*binaryImage(:,:).*Ampmask(:,:,t+2-1);
        threshold=binaryImage(:,:).*Ampmask(:,:,t+2-1);
        %{
      ...
      %}
       ...
       redChannel4 = uint8(orgimg4(:, :, 1)*256);
        ...
        imwrite (outputimg4,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_1.5std\threshold70_2to15\ampmask_phase_',num2str(thresholding),'std\black_T',num2str(bas_num),'_C',num2str(uniquenumber(t)),'Hz.tif'));
       end 
       end 
    %{
    hFig=figure;
