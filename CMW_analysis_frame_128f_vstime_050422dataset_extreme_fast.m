...
%it is not the drifting that matters 
    ...
    fileDirectory = 'F:\800nm OCT computer data\050422 validation\800nm_050422_ex_vivo_validation_oviduct_1000x10000_1x0.1_7us_10ms_4\images\';
 ...
 for bas_num=startnum:endnum
    ...
    redChannel = orgimg1(:, :, 1);
    greenChannel = orgimg1(:, :, 2);
    blueChannel = orgimg1(:, :, 3);
    ...
    outputimg1 = cat(3, redChannel, greenChannel, blueChannel);
    imwrite (outputimg1,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_3std\threshold55_2to15\pure_freq\',fileName1,'pure_freq',num2str(bas_num),'.tif'));
    ...
    imwrite(outputimg2,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_3std\threshold55_2to15\complete_freq\','complete',num2str(bas_num),'.tif'));
    close(hFig);
    ...
    saveas(f, strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_3std\threshold55_2to15\hist\',fileName1,'hist',num2str(bas_num),'.tif'));
    close(f);
    
    
    for freq=3
        mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_3std\threshold55_2to15_',num2str(freq),'Hz\pure_phase'));
    ...
    %for t=2:size(uniquenumber)
        ...
        imwrite (ind2rgb(im2uint8(mat2gray(phaseImage2D)),parula),strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_3std\threshold55_2to15_',num2str(freq),'Hz\pure_phase\',fileName1,'pure_phase_at_freq',num2str(freq),'Hz',num2str(bas_num),'.tif'));
        ...
        colorbar;
        [cdata,colorMap]=getframe(hFig);
        imwrite(cdata,strcat(fileDirectory,'128f_result_3std\threshold55_2to15\complete_phase\',num2str(bas_num),'_',num2str(uniquenumber(t)),'.tif'));
        close(hFig);
        
        imwrite(mat2gray(phaseImage2D),strcat(fileDirectory,'128f_result_3std\threshold55_2to15\pure_phase_gray\',num2str(bas_num),'_',num2str(uniquenumber(t)),'.tif'));
        %}
    end
 end 
