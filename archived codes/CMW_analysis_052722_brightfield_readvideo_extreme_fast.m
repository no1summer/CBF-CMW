    ...
    fileDirectory = 'F:\800nm OCT computer data\052722 validation\Bright Field\';
    fileName1='40x-5-Movie Export-08';
    fileName2 = '.mov';
    Vol(1:1080,1:1920,1:256) = 0;
    
   
    mkdir(strcat(fileDirectory,'256f_3std\threshold115\pure_phase\',fileName1));
    mkdir(strcat(fileDirectory,'256f_3std\threshold115\pure_freq\',fileName1));
    mkdir(strcat(fileDirectory,'256f_3std\threshold115\complete_freq\',fileName1));
    mkdir(strcat(fileDirectory,'256f_3std\threshold115\complete_phase\',fileName1));
    mkdir(strcat(fileDirectory,'256f_3std\threshold115\pure_phase_gray\',fileName1));
    mkdir(strcat(fileDirectory,'256f_3std\threshold115\hist\',fileName1));
    ...
    timeProfile(1:1080,1:1920,1:256) = 0;
    ...
    %figure(3);
    %hold on
    phasevol(1:1080,1:1920,1:129)=0;
    ...
    imwrite (mat2gray(filteredSpectralImage2D),strcat(fileDirectory,'256f_3std\','bright_field_Amp_T',fileName1,num2str(bas_num),'.tif'));
    ...
    imwrite (outputimg1,strcat(fileDirectory,'256f_3std\threshold115\pure_freq\',fileName1,'\bright_field_pure_T',num2str(bas_num),'.tif'));
    ...
    imwrite(outputimg2,strcat(fileDirectory,'256f_3std\threshold115\complete_freq\',fileName1,'\bright_field_complete_T',num2str(bas_num),'.tif'));
    close(hFig);
    %{
    ...
    saveas(f, strcat(fileDirectory,'256f_3std\threshold115\hist\',fileName1,'\hist_T',num2str(bas_num),'.tif'));
    close(f);
      ...
      uniquenumber=unique(frequencyImage);
       for t=2:size(uniquenumber)
                   ...
                   temp = angle(phasevol(:,:,:));
                   phaseImage2D(:,:)=(temp(:,:,round(uniquenumber(t)*NFFT/Fs)+1)+3.14).*binaryImage(:,:);
                   
                
        %phaseImage2D(Indice)=0;
        imwrite (ind2rgb(im2uint8(mat2gray(phaseImage2D)),parula),strcat(fileDirectory,'256f_3std\threshold115\pure_phase\',fileName1,'\freqC',num2str(uniquenumber(t)),'_Z',num2str(bas_num),'.tif'));
        hFig=figure;
        imagesc(phaseImage2D);
        colorbar;
        [cdata,colorMap]=getframe(hFig);
        imwrite(cdata,strcat(fileDirectory,'256f_3std\threshold115\complete_phase\',fileName1,'\freqC',num2str(uniquenumber(t)),'_Z',num2str(bas_num),'.tif'));
        close(hFig);
        
        imwrite(mat2gray(phaseImage2D),strcat(fileDirectory,'256f_3std\threshold115\pure_phase_gray\',fileName1,'\freqC',num2str(uniquenumber(t)),'_Z',num2str(bas_num),'.tif'));
    end
