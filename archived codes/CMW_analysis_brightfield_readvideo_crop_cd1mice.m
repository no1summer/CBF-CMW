    ...
    fileDirectory = 'C:\Users\Larina-Lab-2\Desktop\CD1_04_111921_estrus_40x_5_centreXcentreYtopZ_127fps\CD1_04_111921_estrus_40x_5_centreXcentreYtopZ_127fps.mp4';
  
 
    Vol(1:400,1:600,1:256) = 0;
    
   
    %mkdir(strcat(fileDirectory,'256f_threshold184_wholeImage\threshold160\pure_phase'));
    mkdir(strcat(fileDirectory,'256f_threshold184\threshold160\pure_freq'));
    mkdir(strcat(fileDirectory,'256f_threshold184\threshold160\complete_freq'));
    %mkdir(strcat(fileDirectory,'256f_threshold184_wholeImage\threshold160\complete_phase'));
    %mkdir(strcat(fileDirectory,'256f_threshold184_wholeImage\threshold160\pure_phase_gray'));
    mkdir(strcat(fileDirectory,'256f_threshold184\threshold160\hist'));
    
    v=VideoReader(fileDirectory);
    ...
    for fileNumber = 1:256
       ...
       Vol(:,:,fileNumber)=frame1(651:1050,1301:1900);
       img = mat2gray(frame1(651:1050,1301:1900));
       imwrite(img,strcat(fileDirectory,'256f_threshold184\threshold160\structure.png'));
    ...
    %figure(3);
    %hold on
    phasevol(1:400,1:600,1:endindex)=0;
    ...
    for X = 1:600
        ...
        for D = 1:400
            ...
            phasevol(D,X,1:endindex)= frequencyProfile(1:endindex);
    ...
    %thresholdAmplitude = mean(thresholdVector(thresholdVector~=0))+ 6*std(thresholdVector(thresholdVector~=0));
    ...
    imwrite (ind2rgb(im2uint8(mat2gray(frequencyImage)),hot(256)),strcat(fileDirectory,'256f_threshold184\threshold160\pure_freq\','bright_field_pure_T_hot',num2str(bas_num),'.tif'));
    imwrite (ind2rgb(im2uint8(mat2gray(frequencyImage)),parula),strcat(fileDirectory,'256f_threshold184\threshold160\pure_freq\','bright_field_pure_T',num2str(bas_num),'.tif'));
    hFig=figure;
    ...
    imwrite(rgbImage,strcat(fileDirectory,'256f_threshold184\threshold160\complete_freq\','bright_field_complete_T',num2str(bas_num),'.tif'));
    close(hFig);
    ...
    saveas(f, strcat(fileDirectory,'256f_threshold184\threshold160\hist\','hist_T',num2str(bas_num),'.tif'));
    close(f);
    ...
    %copyfile(strcat(fileDirectory,num2str(bas_num),fileName),strcat(fileDirectory,'result_70endindex6'));
