    ...
    fileName2 = '.tif';
    Vol(1:512,1:100,1:60,1:128) = 0;
    
   
    %mkdir(strcat(fileDirectory,'128f\threshold80_freqindex1to30\pure_phase'));
    %mkdir(strcat(fileDirectory,'new128f_128time\threshold80\pure_freq'));
    mkdir(strcat(fileDirectory,'new_registered_128f_128time\threshold80_freqindex40\pure_freq_color'));
    %mkdir(strcat(fileDirectory,'new_registered_128f_128time\threshold80_freqindex40\binaryImage'));
    %mkdir(strcat(fileDirectory,'128f\threshold80_freqindex1to30\pure_freq_intensity'));
    mkdir(strcat(fileDirectory,'new_registered_128f_128time\threshold80_freqindex40\pure_phase_color_parula'));
 ...
 for j=0:128
    ...
    binaryImage(1:512,1:100,1:60)=0;
    avg=mean(Vol,4);
    imageThreshold = 80;
    ...
    timeProfile(1:128) = 0; 
    phasevol(1:512,1:100,1:60,1:65)=0;
        ...
        for D = 1:512
            ...
            for l=freqN
                if binaryImage(D,X,z)==1
                  phaseProfile = angle(frequencyProfile(1:65));    
                  phasevol(D,X,z,l)=(phaseProfile(l)+pi-freqN*2*pi*z/(129*60));
                  if phasevol(D,X,z,l)<0
                      phasevol(D,X,z,l)=phasevol(D,X,z,l)+2*pi;
                  end 
                  %phasevol(D,X,z,l)=(phaseProfile(l)+3.14);
                else 
                  phasevol(D,X,z,l)=0;  
                end 
            end 
   ...
   %}
    ...
    saveas(f, strcat(fileDirectory,'new128f_128time\threshold80\pure_freq_color\','hist',num2str(j),'.tif'));
    close(f);
    %}
    
    for z=1:60
    imwrite (ind2rgb(im2uint8(mat2gray(freqImage3D(:,:,z))),parula),strcat(fileDirectory,'new_registered_128f_128time\threshold80_freqindex40\pure_freq_color\',fileName1,'pure_T',num2str(j),'_Z',num2str(z),'.tif'));
    %imwrite (mat2gray(binaryImage(:,:,z)),strcat(fileDirectory,'new_registered_128f_128time\threshold80_freqindex40\binaryImage\',fileName1,'pure_T',num2str(j),'_Z',num2str(z),'.tif'));
    %imwrite (mat2gray(freqImage3D(:,:,z)),strcat(fileDirectory,'128f\threshold80_freqindex1to30\pure_freq\','pure_T',num2str(j),'_Z',num2str(z),'.tif'));
    %imwrite (mat2gray(intensity(:,:,z)),strcat(fileDirectory,'128f\threshold80_freqindex1to30\pure_freq_intensity\','pure_T',num2str(j),'_Z',num2str(z),'.tif'));    
    
     for l=freqN
    %imwrite (mat2gray(phasevol(:,:,z,l)),strcat(fileDirectory,'128f\threshold80_freqindex1to30\pure_phase\','pure_T',num2str(j),'_C',num2str(l),'_Z',num2str(z),'.tif'));
     imwrite (ind2rgb(im2uint8(mat2gray(phasevol(:,:,z,l))),parula),strcat(fileDirectory,'new_registered_128f_128time\threshold80_freqindex40\pure_phase_color_parula\',fileName1,'parula_T',num2str(j),'_C',num2str(l),'_Z',num2str(z),'.tif'));
    %imwrite (ind2rgb(im2uint8(mat2gray(phasevol(:,:,z,l))),turbo),strcat(fileDirectory,'128f_128time\threshold40_freqindex30\pure_phase_color_turbo\','turbo_T',num2str(j),'_C',num2str(l),'_Z',num2str(z),'.tif'));
    %imwrite (ind2rgb(im2uint8(mat2gray(phasevol(:,:,z,l))),hsv),strcat(fileDirectory,'128f_128time\threshold40_freqindex30\pure_phase_color_hsv\','hsv_T',num2str(j),'_C',num2str(l),'_Z',num2str(z),'.tif'));
    %imwrite (ind2rgb(im2uint8(mat2gray(phasevol(:,:,z,l))),hot),strcat(fileDirectory,'128f_128time\threshold40_freqindex30\pure_phase_color_hot\','hot_T',num2str(j),'_C',num2str(l),'_Z',num2str(z),'.tif'));
      end
    ...
    for t=2:size(uniquenumber)
        ...
        imwrite (ind2rgb(im2uint8(mat2gray(phaseImage2D)),parula),strcat(fileDirectory,'128f_result_2std\threshold40_2to20\pure_phase\',num2str(bas_num),'_',num2str(uniquenumber(t)),'.tif'));
        hFig=figure;
