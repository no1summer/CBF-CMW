    ...
    for X = 1:1920
        ...
        dlmwrite(strcat(fileDirectory,int2str(X),fileName1,'depthFrequencyImage2D.txt'),depthFrequencyImage2D);
    end   
    %hold off
    ...
    for o = 1:1920
        filteredSpectralImage = dlmread(strcat(fileDirectory,int2str(o),fileName1,'depthFrequencyImage2D.txt'));
        for q = 1:1216
    ...
    imwrite (ind2rgb(im2uint8(mat2gray(frequencyImage)),parula),strcat(fileDirectory,'128f_3std\threshold100_3Hz\pure_freq\',fileName1,'\bright_field_pure_T',num2str(bas_num),'.tif'));
    hFig=figure;
    imagesc(frequencyImage);
    colorbar;
    [cdata,colorMap]=getframe(hFig);
    imwrite(cdata,strcat(fileDirectory,'128f_3std\threshold100_3Hz\complete_freq\',fileName1,'\bright_field_complete_T',num2str(bas_num),'.tif'));
    close(hFig);
    %{
    ...
    saveas(f, strcat(fileDirectory,'128f_3std\threshold100_3Hz\hist\',fileName1,'\hist_T',num2str(bas_num),'.tif'));
    close(f);
        ...
        imwrite (ind2rgb(im2uint8(mat2gray(phaseImage2D)),parula),strcat(fileDirectory,'128f_3std\threshold100_3Hz\pure_phase\',fileName1,'\bright_field_pure_T',num2str(bas_num),'.tif'));
        hFig=figure;
        imagesc(phaseImage2D);
        colorbar;
        [cdata,colorMap]=getframe(hFig);
        imwrite(cdata,strcat(fileDirectory,'128f_3std\threshold100_3Hz\complete_phase\',fileName1,'\bright_field_complete_T',num2str(bas_num),'.tif'));
        close(hFig);
        
        imwrite(mat2gray(phaseImage2D),strcat(fileDirectory,'128f_3std\threshold100_3Hz\pure_phase_gray\',fileName1,'\bright_field_pure_T',num2str(bas_num),'.tif'));
    end
