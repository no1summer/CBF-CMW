    ...
    fileDirectory = 'F:\010623 in vivo\010623_1000x10000_1x0.1_4us_5ms_1\images\';
    fileName1='image_';
    fileName2 = '.tiff';
    ...
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\pure_freq'));
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\hist'));
    mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\complete_freq'));
    ...
    %progressbar(j/100);
        for fileNumber = 1:128
            ...
            tempimg= imread(strcat(fileDirectory,fileName1, num2str(frameNumber,'%06g'),fileName2));
            Vol(:,:,fileNumber)=tempimg(1:512,:);
    ...
    %imwrite (mat2gray(filteredSpectralImage2D),strcat(fileDirectory,'128f_2std_separate\','bright_field_Amp_T',fileName1,num2str(bas_num),'.tif'));
    ...
    imwrite (outputimg1,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\pure_freq\pure_bright_field_pure_T',num2str(bas_num),'.tif'));
    ...
    imwrite(outputimg2,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\complete_freq\bright_field_complete_T',num2str(bas_num),'.tif'));
    close(hFig);
    ...
    saveas(f, strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\hist\hist_T',num2str(bas_num),'.tif'));
    close(f);
      ...
      for thresholding=0.7
       ...
       for t=2:size(uniquenumber)-1
            
       mkdir(strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\ampmask_phase_',num2str(thresholding),'std\'));
       temp = angle(phasevol(:,:,:));
       ...
       redChannel3 = uint8(orgimg3(:, :, 1)*256);
        ...
        imwrite (outputimg3,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\ampmask_phase_',num2str(thresholding),'std\Positive_T',num2str(bas_num),'_C',num2str(uniquenumber(t)),'Hz.tif'));
      %}
       ...
       redChannel4 = uint8(orgimg4(:, :, 1)*256);
        ...
        imwrite (outputimg4,strcat(fileDirectory,num2str(startnum),'to',num2str(endnum),'_128f_result_2std\threshold60_2to20\ampmask_phase_',num2str(thresholding),'std\black_T',num2str(bas_num),'_C',num2str(uniquenumber(t+1)),'Hz.tif'));
       end 
       end 
    %{
    hFig=figure;
    ...
    colorbar;
    [cdata,colorMap]=getframe(hFig);
    imwrite(cdata,strcat(fileDirectory,'128f_2std\threshold130_4Hz\complete_phase\',fileName1,'\bright_field_complete_T',num2str(bas_num),'.tif'));
    close(hFig); 

    imwrite(mat2gray(phaseImage2D),strcat(fileDirectory,'128f_2std\threshold130_4Hz\pure_phase_gray\',fileName1,'\bright_field_pure_T',num2str(bas_num),'.tif'));
    %}
